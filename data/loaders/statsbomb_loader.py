...
def list_competitions() -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(f"Impossible de charger les compétitions StatsBomb : {e}")
        return pd.DataFrame()
...
def load_events(match_id: int) -> pd.DataFrame:
    ...
    try:
        ...
        st.warning(f"Impossible de charger les événements : {e}")
        return pd.DataFrame()
...
def load_lineups(match_id: int) -> dict[str, pd.DataFrame]:
    ...
    try:
        ...
        st.warning(f"Impossible de charger les compositions : {e}")
        return {}
